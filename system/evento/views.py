...
# def list(request):
#     dt_list = []
#     datos = Evento.objects.filter(habilitado=True).all().order_by('-id')
#     for item in datos:
#         dt_list.append(dict(id=item.id,nroEvento=item.nroEvento,
#                             fechaEvento=item.fechaEvento.strftime('%d/%m/%Y'), persona = item.fkpersona.nombre + " " + item.fkpersona.apellidos, fkpersona = item.fkpersona.id,
...
#         dicc["fkusuario"] = user
#         dicc['fechaEvento'] = datetime.datetime.strptime(dicc['fechaEvento'], '%d/%m/%Y')
#         Evento.objects.create(**dicc)
#
...
#         dicc["fkpersona"] = Persona.objects.get(id=dicc["fkpersona"])
#         dicc['fechaEvento'] = datetime.datetime.strptime(dicc['fechaEvento'],'%d/%m/%Y')
#         Evento.objects.filter(pk=dicc["id"]).update(**dicc)
...
#     try:
#         dicc = json.load(request)['obj']
#         obj = Evento.objects.get(id=dicc["id"])
